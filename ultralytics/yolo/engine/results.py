...
class Results(SimpleClass):
    ...
    def __init__(self, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None,use_cuda=True) -> None:
        ...
        self.delete_ind=[]
        self.masks_contour=[]
        self.seletect_feature()


    def seletect_feature(self):
        if not self.boxes:
            return 
        det = self.boxes.cpu().numpy()
        for each_i in range(len(det)):
            mask_each=self.masks[each_i].xy[0].copy().astype(int)
            if not mask_each.any() or mask_each.shape[0]<8:
                    self.delete_ind.append(each_i)
                    continue

            mask_create = np.zeros(self.orig_shape, dtype = np.uint8)
            cv2.fillPoly(mask_create,[mask_each], 1)
            contours, _ = cv2.findContours(mask_create.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
            self.masks_contour.append(contours[0].reshape(-1))
